...
WAIST = TY - TH / 2             # 2.0
FRONT_Z = TZ + TD / 2           # -9.352  (the model faces +Z)
BACK_Z = TZ - TD / 2            # -10.352
...
def shoes(color, kind="shoe", finish=LEATHER):
    ...
    for lg in LEGS:
        ...
        lw, _, ld = lg["size"]
        outward = 1 if lx > TX else -1
        x_lo = lx - lw / 2 - (0.03 if outward < 0 else SEAM)
        x_hi = lx + lw / 2 + (0.03 if outward > 0 else SEAM)
        out.append(rbox(((x_lo + x_hi) / 2, h / 2, lz + 0.10),
                        (x_hi - x_lo, h, ld + 0.16), color,
                        radius=0.075, finish=finish, name="shoe"))
        if kind == "sneaker":
            out.append(rbox((lx, 0.055, lz + 0.10), (lw + 0.10, 0.10, ld + 0.20),
...
def police_cap(color="#1B2740", band="#0C1120", peak="#0A0E18", emblem=(0.86, 0.70, 0.22)):
    ...
    return [
        ...
        rbox((HX, HTOP - 0.01, H_FRONT + 0.15), (0.24, 0.26, 0.05), emblem,
